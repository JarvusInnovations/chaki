...
module.exports = {
    ...
    _installPackages : function (opts) {
        ...
        if (requested) that.App.updateAppJson(iter[0]); // add requested package name to app.json
    ...
    _installPackage : function (opts, fn) {
        ...
        this._getPackageInfo(opts, function (err, packageInfo) {
            var gitPath = packageInfo.data.GitHubPath.replace(/(https?:\/\/)/, ''); // remove the scheme

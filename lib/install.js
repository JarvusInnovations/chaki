...
module.exports = {
    ...
    _installPackages : function (opts) {
        ...
        if (opts.requestedPackage) {
            console.show('a');
            iter.push(opts.requestedPackage);
            requested = true;
        ...
        this.App.addTargetHook(); // add target hook to build.xml
        if (requested) that.App.updateAppJson({packageName : iter[0]}); // add requested package name to app.json
    ...
    _installPackage : function (opts, fn) {
        ...
        this._getPackageInfo(opts, function (err, packageInfo) {
            ...
            var gitPath = packageInfo.data.GitHubPath.replace(/(https?:\/\/)(github.com\/)/, ''); // remove the scheme
                gitPath = gitPath.replace(/(.git$)/, ''); // remove .git
                console.log(">>> ", gitPath);

//            data.path = 'https://github.com/'+packageInfo.data.GitHubPath+'.git';
            data.path = 'https://github.com/'+gitPath+'.git';
    ...
    _getPackageInfo : function (opts, fn) {
        ...
        if (opts.method === "test") {
            ...
            if (this.App.mockApi[packageName]) {
                fn(null, this.App.mockApi[packageName]);
            } else {
                fn("Package not found!");
            }
